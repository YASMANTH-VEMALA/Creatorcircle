...
const PushRegistration: React.FC = () => {
  ...
  const handleNotificationResponse = async (data: any) => {
    try {
      ...
      const { type, relatedPostId, senderId, relatedCommentId, relatedChatId } = data;
      console.log('Notification data:', { type, relatedPostId, senderId, relatedCommentId, relatedChatId });
      ...
      switch (type) {
        ...
        case 'comment_like':
          if (relatedPostId) {
            console.log('Navigating to PostView for post:', relatedPostId);
            navigationRef.current.navigate('PostView' as any, { postId: relatedPostId } as any);
          } else {
        ...
        case 'report_warning':
          if (relatedPostId) {
            console.log('Navigating to PostView for report warning post:', relatedPostId);
            navigationRef.current.navigate('PostView' as any, { postId: relatedPostId } as any);
          } else {
        ...
        case 'follow':
          if (senderId) {
            console.log('Navigating to follower profile:', senderId);
            navigationRef.current.navigate('UserProfile' as any, { userId: senderId } as any);
          } else {
            navigationRef.current.navigate('Notifications' as any);
          }
          break;
        ...
        case 'message':
          if (senderId && user?.uid) {
            try {
              const { default: NewChatService } = await import('./src/services/newChatService');
              // Ensure a chat exists and get its id
              const chatId = await NewChatService.createOrGetChat(user.uid, senderId);
              // Load other user's profile to populate header details
              const otherUser = await NewChatService.getUserProfile(senderId);
              if (otherUser) {
                navigationRef.current.navigate('ChatWindow' as any, { chatId, otherUser } as any);
              } else {
                // Fallback if profile not found
                navigationRef.current.navigate('MessagesList' as any);
              }
            } catch (e) {
              console.warn('Failed to navigate to chat from push:', e);
              navigationRef.current.navigate('MessagesList' as any);
            }
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        // Only request notification permissions after user is logged in
        if (!user?.uid) {
