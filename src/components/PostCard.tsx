...
const PostCard: React.FC<PostCardProps> = React.memo(({ 
  ...
  const getCombinedMedia = () => {
    ...
    if (post.images && post.images.length > 0) {
      ...
      const validImages = post.images.filter(url => 
        url && 
        typeof url === 'string' && 
        url.trim() !== '' && 
        url !== 'undefined' && 
      ...
      media.push(...validImages);
    }
    if (post.videos && post.videos.length > 0) {
      // Filter out invalid URLs
      const validVideos = post.videos.filter(url => 
        url && 
        typeof url === 'string' && 
        url.trim() !== '' && 
        url !== 'undefined' && 
  ...
  const getAvatarSource = () => {
    if (avatarError) {
      return { uri: 'https://ui-avatars.com/api/?name=U&size=40&background=007AFF&color=FFFFFF' };
    }
    ...
    <View style={[styles.container, isInProfile && styles.profileContainer, { 
      ...
      <View style={styles.content}>
        ...
        {getCombinedMedia().length > 0 && (
          ...
          >
          <PostImageStack 
              media={getCombinedMedia().filter(url => url && url.trim() !== '')} 
              ...
              renderItem={({ item }) => (
                <View style={styles.commentItem}>
                  <Image
                    ...
                    defaultSource={{ uri: 'https://ui-avatars.com/api/?name=U&size=30&background=007AFF&color=FFFFFF' }}
                    onLoad={() => {
