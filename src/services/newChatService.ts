...
export class NewChatService {
    ...
    try {
      ...
      try {
        ...
        if (receiverId && receiverId !== senderId) {
          const preview = messageType === 'text' ? (text || '') : (messageType.toUpperCase() + ' attachment');
  ...
  static async sendMediaMessage(
    ...
    try {
      console.log('📸 Sending media message:', { chatId, senderId, receiverId, mediaUri, mediaType });
      
      const messageType = mediaType === 'photo' ? 'image' : 'video';
      const text = `Sent a ${mediaType}`;
  ...
  // Get user's chats (one-time fetch) - simplified to avoid index requirements
  static async getUserChats(userId: string): Promise<Chat[]> {
    try {
      // Get all chats and filter on client side to avoid index requirements
      const snapshot = await getDocs(collection(db, 'chats'));
      const chats: Chat[] = [];
      
      snapshot.forEach((doc) => {
        const data = doc.data() as Chat;
        // Check if user is participant and chat is not deleted for this user
        if (data.participants?.includes(userId) && !data.isDeleted?.[userId]) {
  ...
  static async setTyping(chatId: string, userId: string, isTyping: boolean): Promise<void> {
    try {
      ...
      if (chatDoc.exists()) {
        ...
        let typingUsers = Array.isArray(data?.typingUsers) ? data.typingUsers : [];
  ...
  static async searchMessages(chatId: string, searchText: string): Promise<ChatMessage[]> {
    try {
      ...
      snapshot.forEach((doc) => {
        ...
        if (message.text && typeof message.text === 'string' && message.text.toLowerCase().includes(searchText.toLowerCase()) && !message.isDeleted) {
  ...
  // Theme persistence methods - Fixed export issue
  static async saveChatTheme(chatId: string, theme: ChatTheme): Promise<void> {
    try {
      const themeRef = doc(db, 'chatThemes', chatId);
      await setDoc(themeRef, {
        ...theme,
        savedAt: serverTimestamp(),
      });
      console.log('✅ Theme saved for chat:', chatId);
    } catch (error) {
  ...
  static async getChatTheme(chatId: string): Promise<ChatTheme | null> {
    try {
      const themeRef = doc(db, 'chatThemes', chatId);
      ...
      if (themeDoc.exists()) {
        ...
        return {
          ...
          bubbleTextColor: themeData.bubbleTextColor,
          inputBackground: themeData.inputBackground || '#F2F2F7',
          placeholderColor: themeData.placeholderColor || '#8E8E93',
        } as ChatTheme;
      }
      return null;
    } catch (error) {
  ...
  static async markMessageAsSeen(chatId: string, messageId: string): Promise<void> {
    try {
      const messageRef = doc(db, 'chats', chatId, 'messages', messageId);
      await updateDoc(messageRef, {
        isSeen: true,
  ...
  // Mark multiple messages as seen (for when user opens chat)
  static async markMessagesAsSeen(chatId: string, messageIds: string[]): Promise<void> {
    try {
      const batch = writeBatch(db);
      const seenTimestamp = serverTimestamp();
      ...
      await batch.commit();
      console.log(`✅ ${messageIds.length} messages marked as seen in chat:`, chatId);
    } catch (error) {
  ...
  static async getUnseenMessages(chatId: string, userId: string): Promise<ChatMessage[]> {
    try {
      const messagesRef = collection(db, 'chats', chatId, 'messages');
      const q = query(
        messagesRef,
      ...
      snapshot.forEach((doc) => {
        unseenMessages.push({ ...doc.data(), id: doc.id } as ChatMessage);
      });
      
      console.log(`📬 Found ${unseenMessages.length} unseen messages for user:`, userId);
