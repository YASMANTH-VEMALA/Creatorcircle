...
const FindPeopleScreen: React.FC = () => {
  ...
  const isPlaceholderBanner = (url: string | undefined) => {
    if (!url) return false;
  ...
  const renderUserItem = ({ item }: { item: Profile }) => (
    ...
    >
      {/* Banner Image */}
      {/* Banner: show only real user-uploaded images; skip placeholders */}
      {item.bannerPhotoUrl && !isPlaceholderBanner(item.bannerPhotoUrl) ? (
        <View style={styles.bannerContainer}>
          <Image 
            source={{ uri: item.bannerPhotoUrl }} 
            ...
            onError={() => {
              ...
              console.warn('FindPeople banner failed to load');
            }}
          />
        </View>
      ) : null}
