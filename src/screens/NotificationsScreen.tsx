...
const NotificationsScreen: React.FC = () => {
  ...
  const loadNotifications = useCallback(async () => {
    ...
    try {
      const unsubscribe = notificationService.listenToNotifications(user.uid, (notifications) => {
        ...
        const convertedNotifications: NotificationItem[] = notifications
          .filter(n => !n.read)
          .map(notification => ({
          id: notification.id,
          ...
          relatedChatId: (notification as any).relatedChatId,
        }));
  ...
  const handleNotificationPress = useCallback(async (item: NotificationItem) => {
    ...
    // Mark as read immediately and optimistically remove from list
    ...
    switch (item.type) {
      ...
      case 'comment_like':
        if (item.relatedPostId) {
          console.log('Navigating directly to PostView for:', item.relatedPostId);
      ...
      case 'message':
        if (item.senderId && user?.uid) {
          try {
            const NewChatService = (await import('../services/newChatService')).default;
            const chatId = await NewChatService.createOrGetChat(user.uid, item.senderId);
            const otherUser = await NewChatService.getUserProfile(item.senderId);
            if (otherUser) {
              navigation.navigate('ChatWindow' as never, { chatId, otherUser } as never);
            } else {
    ...
    return (
      ...
      >
        <TouchableOpacity 
          ...
          onPress={() => handleUserProfilePress(item.senderId)}
          activeOpacity={0.7}
        >
