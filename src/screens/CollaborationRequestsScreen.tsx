...
import { CollaborationRequest } from '../services/collaborationService';
...
const CollaborationRequestsScreen: React.FC = () => {
    ...
    try {
      ...
      const unsubscribe = collaborationService.listenToPendingRequests(user.uid, async (newRequests) => {
        ...
        for (const request of newRequests) {
          if (!profiles[request.senderId]) {
            try {
              // Fetch requester profile via UserService
              const profile = await UserService.getUserProfile(request.senderId);
              if (profile) {
                profiles[request.senderId] = profile;
              }
            } catch (error) {
              console.error(`Error loading profile for user ${request.senderId}:`, error);
              // Create a fallback profile to prevent crashes
  ...
  const handleAccept = async (request: CollaborationRequest) => {
    try {
      ...
      Alert.alert(
        'Request Accepted',
        `${requestProfiles[request.senderId]?.name || 'User'} accepted your collaboration request!`
      );
    } catch (error) {
  ...
  const handleDecline = async (request: CollaborationRequest) => {
    Alert.alert(
      'Decline Request',
      `Are you sure you want to decline the collaboration request from ${requestProfiles[request.senderId]?.name || 'this user'}?`,
      [
  ...
  const renderRequestItem = ({ item }: { item: CollaborationRequest }) => {
    const requesterProfile = requestProfiles[item.senderId];
      ...
      <View style={styles.requestItem}>
        <View style={styles.userInfo}>
          ...
          <View style={styles.userDetails}>
            ...
            <Text style={styles.requestTime}>{formatTime(item.createdAt?.toDate())}</Text>
          </View>
        </View>
