...
const ProfileScreen: React.FC = () => {
    ...
    try {
      ...
      if (userProfile) {
        ...
        console.log('📋 Profile loaded:', {
          ...
          lastStreakWindowStart: userProfile.lastStreakWindowStart
        });
      }
      
      // Set default verified badge (no premium service)
      setVerifiedBadge('none');
      console.log('🏆 Verified badge disabled (premium features coming soon)');
    } catch (error) {
  ...
  return (
    ...
    >
        ...
        {(() => {
          ...
          return (
            <View>
              ...
              <View style={styles.progressBar}>
                <View style={[styles.progressFill, { width: `${progress}%`, backgroundColor: '#FFD700' }]} />
              </View>
              ...
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                <Ionicons name={showXpLog ? 'chevron-down' : 'chevron-forward'} size={18} color="#FFD700" />
                <Text style={{ color: '#FFD700', fontWeight: '600', marginLeft: 4 }}>XP Activity</Text>
              </TouchableOpacity>
              {showXpLog && (
      ...
      <View style={styles.section}>
        ...
        {isEditing ? (
          <TouchableOpacity
            ...
            onPress={() => setShowPersonalityDropdown(true)}
            accessibilityLabel="Select personality type"
            accessibilityHint="Opens personality type selection"
          >
            <Text style={[
              styles.personalityDropdownText,
              !profile.personality && styles.personalityPlaceholder
            ]}>
              {profile.personality ? profile.personality.charAt(0).toUpperCase() + profile.personality.slice(1) : 'Select personality type'}
            </Text>
            <Ionicons name="chevron-down" size={20} color="#666" />
          </TouchableOpacity>
        ) : (
          <Text style={styles.personalityText}>
            {profile.personality ? profile.personality.charAt(0).toUpperCase() + profile.personality.slice(1) : 'Not specified'}
          </Text>
        )}
      </View>
                ...
                <ScrollView keyboardShouldPersistTaps="handled" style={{ maxHeight: 160 }}>
                  ...
                  {!(profile.skills || []).some(x => x.toLowerCase() === newSkill.trim().toLowerCase()) && (
                    <TouchableOpacity style={styles.suggestionItem} onPress={addSkill}>
                      <Ionicons name="add-outline" size={16} color="#FFD700" />
                      <Text style={[styles.suggestionText, { color: '#FFD700' }]}>Add "{newSkill.trim()}"</Text>
                    </TouchableOpacity>
                  )}
                </ScrollView>
              </View>
          ...
          <View>
            ...
            {newInterest.trim().length > 0 && (
              <View style={styles.suggestionsPanel}>
                <ScrollView keyboardShouldPersistTaps="handled" style={{ maxHeight: 160 }}>
                  {predefinedInterests
                    .filter(i => i.toLowerCase().includes(newInterest.trim().toLowerCase()))
                    .filter(i => !(profile.interests || []).some(x => x.toLowerCase() === i.toLowerCase()))
                    .map((i) => (
                      <TouchableOpacity key={i} style={styles.suggestionItem} onPress={() => {
                        setProfile(prev => ({ ...prev, interests: [ ...(prev.interests || []), i ] }));
                        setNewInterest('');
                      }}>
                  ...
                  {/* Add custom option */}
                  {!(profile.interests || []).some(x => x.toLowerCase() === newInterest.trim().toLowerCase()) && (
                    <TouchableOpacity style={styles.suggestionItem} onPress={addInterest}>
                      <Ionicons name="add-outline" size={16} color="#ff6b6b" />
                      <Text style={[styles.suggestionText, { color: '#ff6b6b' }]}>Add "{newInterest.trim()}"</Text>
                    </TouchableOpacity>
                  )}
                </ScrollView>
              </View>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
              {(profile.socialLinks || []).map((link, idx) => (
                <View key={`${link.platform}-${idx}`} style={styles.socialPill}>
                  <Ionicons name={platformIconName(link.platform)} size={16} color="#FFD700" />
      ...
      <Modal
        visible={showPersonalityDropdown}
        transparent={true}
        animationType="fade"
        onRequestClose={() => setShowPersonalityDropdown(false)}
      >
        <TouchableOpacity
          style={styles.modalOverlay}
          activeOpacity={1}
          onPress={() => setShowPersonalityDropdown(false)}
        >
          <View style={styles.personalityModal}>
            <Text style={styles.personalityModalTitle}>Select Personality Type</Text>
            {['introvert', 'extrovert', 'ambivert'].map((type) => (
              <TouchableOpacity
                key={type}
                style={[
                  styles.personalityOption,
                  profile.personality === type && styles.personalityOptionSelected
                ]}
                onPress={() => {
                  setProfile(prev => ({ ...prev, personality: type as 'introvert' | 'extrovert' | 'ambivert' }));
              ...
              >
                <Text style={[
                  ...
                  profile.personality === type && styles.personalityOptionTextSelected
                ]}>
      ...
      >
        <View style={styles.successModalOverlay}>
          <View style={styles.successModal}>
            ...
            <Text style={styles.successModalText}>Profile updated successfully!</Text>
            
            {/* Profile Completion Status */}
            <View style={styles.completionStatusContainer}>
              <View style={styles.completionStatusHeader}>
                <Ionicons 
                  name={completionStatus.isComplete ? "trophy" : "trending-up"} 
                  size={20} 
                  color={completionStatus.isComplete ? "#FFD700" : "#FF9500"} 
                />
                <Text style={styles.completionStatusTitle}>
                  {completionStatus.isComplete ? "Profile Complete!" : "Profile Progress"}
                </Text>
              </View>
              ...
              <View style={styles.completionPercentageContainer}>
                ...
                <Text style={styles.completionPercentageLabel}>Complete</Text>
              </View>
              ...
              <View style={styles.completionProgressBar}>
                <View 
                  style={[
                    ...
                    { 
                      width: `${completionStatus.completionPercentage}%`,
                      backgroundColor: completionStatus.isComplete ? "#4CAF50" : "#FFD700"
                    }
                  ]} 
                />
              </View>
              ...
              {!completionStatus.isComplete && completionStatus.missingFields.length > 0 && (
                <View style={styles.missingFieldsContainer}>
                  <Text style={styles.missingFieldsTitle}>Complete these to reach 100%:</Text>
                  {completionStatus.missingFields.slice(0, 3).map((field, index) => (
                    <Text key={index} style={styles.missingFieldItem}>
                      • {field}
                    </Text>
                  ))}
                  {completionStatus.missingFields.length > 3 && (
                    <Text style={styles.missingFieldItem}>
