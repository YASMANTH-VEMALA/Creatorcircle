...
const PostCreationModal: React.FC<PostCreationModalProps> = ({
  ...
  const loadUserProfile = useCallback(async () => {
    try {
      ...
      if (!profile) {
        ...
        Alert.alert('Profile Error', 'Unable to load user profile. Please try again.');
        return;
      }
      ...
      Alert.alert('Profile Error', 'Failed to load user profile. Please try again.');
    }
  }, [user?.uid]);
  ...
  const pickVideo = async () => {
    try {
      ...
      if (videos.length >= MAX_VIDEOS) {
        Alert.alert('Video Limit Reached', `You can only upload ${MAX_VIDEOS} video per post`);
        return;
      }

      const result = await ImagePicker.launchImageLibraryAsync({
        mediaTypes: ImagePicker.MediaTypeOptions.Videos,
        allowsMultipleSelection: false, // Only allow single video selection
        quality: 0.8,
        videoMaxDuration: 60, // 60 seconds max
      });

      if (!result.canceled && result.assets && result.assets[0]) {
        // Replace any existing video with the new one (since we only allow 1 video)
  ...
  const handleCreatePost = async () => {
    ...
    try {
      ...
      // 🛡️ PRE-CREATION CONTENT MODERATION WITH VERIFICATION STEPS
      setVerificationStep(0);
      // Step 1: Analyzing text content
      setVerificationStatus('🔍 Analyzing text content...');
      setVerificationStep(1);
      console.log('🔍 Checking content before posting...');
      ...
      if (images.length > 0) {
        setVerificationStatus('🖼️ Analyzing images...');
        setVerificationStep(2);
      }
      
      // Step 3: Analyzing videos
      if (videos.length > 0) {
        setVerificationStatus('🎥 Analyzing videos...');
        setVerificationStep(3);
      }
      
      // Step 4: AI-powered analysis
      setVerificationStatus('🤖 AI-powered content analysis...');
      setVerificationStep(4);
      
      const moderationResult = StrictContentModeration.analyzePost(content, images, videos);
      console.log('AI Analysis result in modal:', moderationResult);
      
      if (!moderationResult.isAppropriate) {
        console.log('❌ Content blocked by AI analysis:', moderationResult.reasons);
        ...
        Alert.alert(
          ...
          `Your post contains inappropriate content and cannot be published.\n\nAI Analysis Results:\n• Category: ${moderationResult.category.toUpperCase()}\n• Confidence: ${Math.round(moderationResult.confidence * 100)}%\n• Issues: ${moderationResult.reasons.join('\n• ')}\n\nPlease review and modify your content to follow our community guidelines.`,
          [{ text: 'OK' }]
        );
        return;
      }

      // Step 5: Content approved
      setVerificationStatus('✅ Content approved, creating post...');
      ...
      if (error.message && error.message.includes('Post blocked')) {
        Alert.alert('Content Blocked', error.message);
      } else if (error.message && error.message.includes('User profile not found')) {
        Alert.alert('Profile Error', 'User profile not found. Please complete your profile first.');
      } else if (error.message && error.message.includes('Unwanted content')) {
        Alert.alert('Content Blocked', error.message);
      } else {
  ...
  return (
    ...
    >
      <View style={styles.container}>
          ...
          <View style={styles.textInputContainer}>
            ...
            <View style={styles.inputFooter}>
              <Text style={styles.optionalText}>
                Caption is optional - you can post with just media
              </Text>
        ...
        {/* Verification Status */}
        {isCreating && verificationStatus && (
          <View style={styles.verificationContainer}>
            <View style={styles.verificationContent}>
              <ActivityIndicator size="small" color="#007AFF" style={styles.verificationSpinner} />
              ...
              {[1, 2, 3, 4, 5].map((step) => (
                <View
                  ...
                  style={[
                    ...
                    step <= verificationStep && styles.verificationStepActive
                  ]}
                />
              ))}
            </View>
