...
const PostImageStack: React.FC<PostImageStackProps> = ({ media, onImagePress }) => {
  ...
  const getDisplayUri = (uri: string): string => {
    // Swap unreliable placeholder domains with a more reliable one
    if (!uri || uri.includes('via.placeholder.com')) {
      // Use placehold.co which serves proper PNGs with correct headers
      return 'https://placehold.co/600x600/png?text=Image';
    }
    return uri;
  };
  ...
  const handleImageError = (index: number) => {
    const imageUrl = media[index];
    console.error(`❌ PostImageStack: Image ${index} failed to load: ${imageUrl}`);
    
    // Check if it's a placeholder URL that might be having issues
    if (imageUrl.includes('via.placeholder.com') || imageUrl.includes('picsum.photos')) {
      console.warn(`⚠️ PostImageStack: Placeholder URL failed to load: ${imageUrl}`);
      console.warn(`💡 This might be a network issue or the placeholder service is down`);
    }
    
    setImageLoading(prev => ({ ...prev, [index]: false }));
    setImageErrors(prev => ({ ...prev, [index]: true }));
    
    // Try to reload the image after a delay, but only if it's not a placeholder URL
    if (!imageUrl.includes('via.placeholder.com') && !imageUrl.includes('picsum.photos')) {
      setTimeout(() => {
        setImageErrors(prev => ({ ...prev, [index]: false }));
        setImageLoading(prev => ({ ...prev, [index]: true }));
      ...
      return (
        <View style={styles.mediaContainer}>
          <Video
            ...
            shouldPlay={false}
            onError={(error) => {
              VideoErrorHandler.handleVideoError(error, uri, `PostImageStack-${index}`);
              handleImageError(index);
            }}
        ...
        {imageErrors[index] && (
          <View style={styles.errorOverlay}>
            ...
            <Text style={styles.errorText}>
              {media[index].includes('via.placeholder.com') || media[index].includes('picsum.photos') 
...
const styles = StyleSheet.create({
  ...
  mediaContainer: {
    width: '100%',
    height: width, // make square 1:1 with screen width
    borderRadius: 0,
    overflow: 'hidden',
    position: 'relative',
