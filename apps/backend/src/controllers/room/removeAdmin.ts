    ...
    try {
        ...
        if (!room.creatorId.equals(currentUser._id)) {
            res.status(403).json({
                success: false,
                error: 'Forbidden',
                message: 'Only the room creator can remove admin privileges'
            });
            return;
        }

        const memberToDemoteId = new Types.ObjectId(userId);
        await room.removeAdmin(memberToDemoteId);

        logger.info(`Admin privileges removed from ${userId} in room ${id} by ${uid}`);
