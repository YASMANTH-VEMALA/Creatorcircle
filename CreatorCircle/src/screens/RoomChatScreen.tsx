...
const RoomChatScreen: React.FC = () => {
  ...
  // Load sender profiles when messages change
  useEffect(() => {
    if (messages.length > 0) {
      loadMessageSenderProfiles(messages);
    }
  }, [messages]);
  ...
  const loadRoomData = async () => {
    try {
      ...
      const unsubMessages = roomService.subscribeToRoomMessages(roomId, (messagesData) => {
        ...
        loadMessageSenderProfiles(messagesData);
      });
  ...
  const loadMessageSenderProfiles = async (messagesData: RoomMessage[]) => {
    try {
      // Get unique sender IDs from messages
      const senderIds = [...new Set(messagesData.map(msg => msg.senderId))];
      
      // Load profiles for senders not already in memberProfiles
      const existingSenderIds = memberProfiles.map(profile => profile.uid);
      const newSenderIds = senderIds.filter(id => !existingSenderIds.includes(id));
      
      if (newSenderIds.length > 0) {
        const newProfiles = await Promise.all(
          newSenderIds.map(senderId => UserService.getUserProfile(senderId))
        );
        
        // Add new profiles to existing memberProfiles
        const validNewProfiles = newProfiles.filter(Boolean) as Profile[];
        setMemberProfiles(prev => [...prev, ...validNewProfiles]);
      }
    } catch (error) {
      console.error('Error loading message sender profiles:', error);
    }
  };
  ...
  const renderMessage = ({ item }: { item: RoomMessage }) => {
    ...
    // If we don't have the sender profile yet, try to load it
    if (!senderProfile && !isOwnMessage) {
      loadMessageSenderProfiles([item]);
    }
